...
class MembershipManagerTest extends TestCase {
	...
	public static function setUpBeforeClass() {
		...
		MembershipManagerTest::clearDB();

		// create test groups and accounts
		for ($i = 1; $i <= 4; $i++) {
			MembershipManagerTest::createBackendGroup($i);
			MembershipManagerTest::createAccount($i);
		}
	}
	...
	public function testRegularAccountOperations() {
		...
		for ($i = 1; $i <= 2; $i++) {
			$account = MembershipManagerTest::getAccount($i);
		...
		$account = MembershipManagerTest::getAccount(3);
		$this->assertInstanceOf(Account::class, $account);
		$this->addAdmin($account, $group2);

		// Check adding as only USER
		$account = MembershipManagerTest::getAccount(4);
		$this->assertInstanceOf(Account::class, $account);
	...
	public function testGetConsistency() {
		$accountId = 1;
		$groupId = 1;
		$account = MembershipManagerTest::getAccount($accountId);
		$group = MembershipManagerTest::getBackendGroup($groupId);
	...
	public function testRemoveAccountOperations() {
		$result = $this->manager->getAdminAccounts();
		$this->assertEmpty($result);
		$group = MembershipManagerTest::getBackendGroup(1);
		...
		$account = MembershipManagerTest::getAccount(1);
		...
		$this->assertCount(3, $this->manager->getAdminAccounts());
		...
		$account = MembershipManagerTest::getAccount(2);
		$this->assertTrue($this->manager->isGroupAdmin($account->getUserId(), $group->getGroupId()));
		$this->assertTrue($this->manager->isGroupUser($account->getUserId(), $group->getGroupId()));
		...
		$this->assertTrue($result);
		$this->assertCount(2, $this->manager->getAdminAccounts());
	...
	public function testRemoveIncorrectIds() {
		...
		$this->assertCount(4, $this->manager->getAdminAccounts());
		$this->assertCount(4, $this->manager->getGroupUserAccounts($group->getGroupId()));
	...
	public function testRemoveFailed() {
		...
		try {
			...
			$thrown = true;
			$this->assertInstanceOf(\Doctrine\DBAL\Exception\ForeignKeyConstraintViolationException::class, $e);
		}
	...
	private function cleanupGroups() {
		...
		for ($i = 1; $i <= 4; $i++) {
			$group = MembershipManagerTest::getBackendGroup($i);
